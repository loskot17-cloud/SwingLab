...
export function drawDrill(state){
  ...
  if(activeDrill.type==='vline'){
    ctx.fillStyle=p.col;ctx.fillRect(p.x*state.poseCanvas.width-p.zw,0,p.zw*2,state.poseCanvas.height);
    ctx.setLineDash([5,4]);ctx.lineWidth=1.5;ctx.strokeStyle=p.col.replace('0.4','0.7');
    ctx.beginPath();ctx.moveTo(p.x*state.poseCanvas.width,0);ctx.lineTo(p.x*state.poseCanvas.width,state.poseCanvas.height);ctx.stroke();ctx.setLineDash([]);
    ctx.font='600 9px var(--mono)';ctx.fillStyle=p.col.replace('0.4','0.8');ctx.textAlign='center';ctx.fillText(p.lbl,p.x*state.poseCanvas.width,16);
  } else if(activeDrill.type==='vzone'){
    const w = state.poseCanvas.width, h = state.poseCanvas.height;
    ctx.fillStyle=p.col;ctx.strokeStyle=p.bc;ctx.lineWidth=1.5;ctx.setLineDash([4,3]);
    ctx.fillRect(p.x*w-p.w*w/2,p.y*h,p.w*w,p.h*h);ctx.strokeRect(p.x*w-p.w*w/2,p.y*h,p.w*w,p.h*h);ctx.setLineDash([]);
    ctx.font='600 8px var(--mono)';ctx.fillStyle=p.bc;ctx.textAlign='center';ctx.fillText(p.lbl,p.x*w,p.y*h-4);
  } else if(activeDrill.type==='eline'){
    ctx.setLineDash([6,4]);ctx.lineWidth=2;ctx.strokeStyle=p.col;
    ctx.beginPath();ctx.moveTo(p.x1*state.poseCanvas.width,p.y1*state.poseCanvas.height);ctx.lineTo(p.x2*state.poseCanvas.width,p.y2*state.poseCanvas.height);ctx.stroke();ctx.setLineDash([]);
    ctx.font='600 8px var(--mono)';ctx.fillStyle=p.col.replace('0.4','0.8');ctx.textAlign='center';ctx.fillText(p.lbl,(p.x1+p.x2)/2*state.poseCanvas.width,Math.min(p.y1,p.y2)*state.poseCanvas.height-6);
  } else if(activeDrill.type==='slot'){
    const w = state.poseCanvas.width, h = state.poseCanvas.height;
    const dx=p.x2-p.x1,dy=p.y2-p.y1,l=Math.hypot(dx,dy),nx=-dy/l*p.sw/2,ny=dx/l*p.sw/2;
    ctx.fillStyle=p.col;
    ctx.beginPath();ctx.moveTo((p.x1+nx)*w,(p.y1+ny)*h);ctx.lineTo((p.x2+nx)*w,(p.y2+ny)*h);ctx.lineTo((p.x2-nx)*w,(p.y2-ny)*h);ctx.lineTo((p.x1-nx)*w,(p.y1-ny)*h);ctx.closePath();ctx.fill();
    ctx.strokeStyle=p.bc;ctx.lineWidth=1;ctx.setLineDash([3,3]);ctx.stroke();ctx.setLineDash([]);
    ctx.font='600 8px var(--mono)';ctx.fillStyle=p.bc;ctx.textAlign='center';ctx.fillText(p.lbl,(p.x1+p.x2)/2*w+p.sw,(p.y1+p.y2)/2*h);
  }
  ctx.restore();
}
