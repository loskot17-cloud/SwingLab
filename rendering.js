...
export function drawDrill(state){
  ...
  const { activeDrill, analysisResult, allFrameData, currentFrameIdx, ctx, videoElement, _cameraMode } = state;
  ...
  if (videoElement && !_cameraMode) {
    const vw = videoElement.videoWidth || state.poseCanvas.width;
    const vh = videoElement.videoHeight || state.poseCanvas.height;
    scale = Math.max(state.poseCanvas.width / vw, state.poseCanvas.height / vh);
    offsetX = (state.poseCanvas.width - vw * scale) / 2;
    offsetY = (state.poseCanvas.height - vh * scale) / 2;
  }
  const transformX = (x) => x * (videoElement?.videoWidth || state.poseCanvas.width) * scale + offsetX;
  const transformY = (y) => y * (videoElement?.videoHeight || state.poseCanvas.height) * scale + offsetY;
  let p;
  ...
  if(activeDrill.type==='vline'){
    ctx.fillStyle=p.col;ctx.fillRect(transformX(p.x) - p.zw * scale, 0, p.zw * 2 * scale, state.poseCanvas.height);
    ctx.setLineDash([5,4]);ctx.lineWidth=1.5;ctx.strokeStyle=p.col.replace('0.4','0.7');
    ctx.beginPath();ctx.moveTo(transformX(p.x),0);ctx.lineTo(transformX(p.x),state.poseCanvas.height);ctx.stroke();ctx.setLineDash([]);
    ctx.font='600 9px var(--mono)';ctx.fillStyle=p.col.replace('0.4','0.8');ctx.textAlign='center';ctx.fillText(p.lbl,transformX(p.x),16);
  } else if(activeDrill.type==='vzone'){
    const x = transformX(p.x - p.w/2), y = transformY(p.y), width = p.w * (videoElement?.videoWidth || state.poseCanvas.width) * scale, height = p.h * (videoElement?.videoHeight || state.poseCanvas.height) * scale;
    ...
    ctx.font='600 8px var(--mono)';ctx.fillStyle=p.bc;ctx.textAlign='center';ctx.fillText(p.lbl,transformX(p.x),transformY(p.y)-4);
  } else if(activeDrill.type==='eline'){
    ctx.setLineDash([6,4]);ctx.lineWidth=2;ctx.strokeStyle=p.col;
    ctx.beginPath();ctx.moveTo(transformX(p.x1),transformY(p.y1));ctx.lineTo(transformX(p.x2),transformY(p.y2));ctx.stroke();ctx.setLineDash([]);
    ctx.font='600 8px var(--mono)';ctx.fillStyle=p.col.replace('0.4','0.8');ctx.textAlign='center';ctx.fillText(p.lbl,transformX((p.x1+p.x2)/2),transformY(Math.min(p.y1,p.y2))-6);
  } else if(activeDrill.type==='slot'){
    const x = transformX(p.x - p.w/2), y = transformY(p.y), width = p.w * (videoElement?.videoWidth || state.poseCanvas.width) * scale, height = p.h * (videoElement?.videoHeight || state.poseCanvas.height) * scale;
    const dx = transformX(p.x2) - transformX(p.x1), dy = transformY(p.y2) - transformY(p.y1), ln = Math.hypot(dx,dy), nx = -dy/ln * p.sw * scale / 2, ny = dx/ln * p.sw * scale / 2;
    ctx.fillStyle=p.col;
    ctx.beginPath();ctx.moveTo(x + nx, y + ny);ctx.lineTo(x + width + nx, y + ny);ctx.lineTo(x + width - nx, y + height - ny);ctx.lineTo(x - nx, y + height - ny);ctx.closePath();ctx.fill();
    ctx.strokeStyle=p.bc;ctx.lineWidth=1;ctx.setLineDash([3,3]);ctx.stroke();ctx.setLineDash([]);
    ctx.font='600 8px var(--mono)';ctx.fillStyle=p.bc;ctx.textAlign='center';ctx.fillText(p.lbl,transformX((p.x1+p.x2)/2) + p.sw * scale,transformY((p.y1+p.y2)/2));
  }
  ctx.restore();
}
...
export function redraw(state){
  ...
  const transformX = (x) => x * (videoElement?.videoWidth || w) * scale + offsetX;
  const transformY = (y) => y * (videoElement?.videoHeight || h) * scale + offsetY;
  ...
  if(showGhost&&analysisResult){
    ...
    if(idealLm){
      ...
      for(const [a,b] of conn){if(idealLm[a].visibility>0.3 && idealLm[b].visibility>0.3){ctx.beginPath();ctx.moveTo(transformX(idealLm[a].x),transformY(idealLm[a].y));ctx.lineTo(transformX(idealLm[b].x),transformY(idealLm[b].y));ctx.stroke()}}
      ctx.setLineDash([]);ctx.lineWidth=1;ctx.strokeStyle='rgba(255,255,255,0.4)';ctx.fillStyle='rgba(255,255,255,0.1)';
      for(const i of [11,12,13,14,15,16,23,24,25,26,27,28]){if(idealLm[i].visibility>0.3){ctx.beginPath();ctx.arc(transformX(idealLm[i].x),transformY(idealLm[i].y),4,0,Math.PI*2);ctx.fill();ctx.stroke()}}
      ctx.globalAlpha=0.45;ctx.font='500 9px var(--mono)';ctx.fillStyle='#fff';ctx.textAlign='left';
      ctx.fillText('IDEAL '+phase.toUpperCase(),transformX(Math.min(idealLm[L.LEFT_SHOULDER].x,idealLm[L.RIGHT_SHOULDER].x)),transformY(Math.min(idealLm[L.LEFT_SHOULDER].y,idealLm[L.RIGHT_SHOULDER].y))-12);
      ctx.restore();
    }
  }
  ...
  if(showSkeleton){
    ...
    for(let i = 0; i < state.previousLandmarks.length - 1; i++){
      ...
      for(const [a,b] of conn){
        if(prevLm[a].visibility>0.3&&prevLm[b].visibility>0.3){
          ctx.beginPath();ctx.moveTo(transformX(prevLm[a].x),transformY(prevLm[a].y));ctx.lineTo(transformX(prevLm[b].x),transformY(prevLm[b].y));ctx.stroke();
        }
      }
      ctx.restore();
    }
    ...
    for(let i = 0; i < silhouettePoints.length; i++){
      ...
      if(pt.visibility > 0.3){
        if(i === 0) ctx.moveTo(transformX(pt.x), transformY(pt.y));
  ...
  if(showSkeleton){
    ...
    for(const [a,b] of conn){
      if(lm[a].visibility>0.3&&lm[b].visibility>0.3){
        ...
        if(g!=='good'){ctx.lineWidth=6;ctx.strokeStyle=g==='warn'?'rgba(251,191,36,0.1)':'rgba(248,113,113,0.12)';ctx.beginPath();ctx.moveTo(transformX(lm[a].x),transformY(lm[a].y));ctx.lineTo(transformX(lm[b].x),transformY(lm[b].y));ctx.stroke();}
        ctx.lineWidth=g==='good'?1.8:2.2;
        ctx.strokeStyle=g==='good'?'rgba(74,222,128,0.75)':g==='warn'?'rgba(251,191,36,0.8)':'rgba(248,113,113,0.8)';
        ctx.beginPath();ctx.moveTo(transformX(lm[a].x),transformY(lm[a].y));ctx.lineTo(transformX(lm[b].x),transformY(lm[a].y));ctx.lineTo(transformX(lm[b].x),transformY(lm[b].y));ctx.stroke();
      }
    }
    const jg={};
    ...
    for(const i of [11,12,13,14,15,16,23,24,25,26,27,28]){
      if(lm[i].visibility>0.3){const g=jg[i]||'good';
        ...
        ctx.shadowBlur = 4;
        ctx.beginPath();ctx.arc(transformX(lm[i].x),transformY(lm[i].y),g==='good'?2.5:3.5,0,Math.PI*2);ctx.fillStyle=g==='good'?'rgba(74,222,128,0.9)':g==='warn'?'rgba(251,191,36,0.9)':'rgba(248,113,113,0.9)';ctx.fill();
        ctx.restore();
      }
    }
  }
  ...
  if(showPlane&&detectedView==='dtl'&&analysisResult?.phases){
    ...
    if(aF){
      ...
      ctx.beginPath();ctx.moveTo(transformX(hp.x-ex*0.3),transformY(hp.y-ey*0.3));ctx.lineTo(transformX(sh.x+ex*0.5),transformY(sh.y+ey*0.5));ctx.stroke();ctx.setLineDash([]);}
      ctx.restore();
    }
  }
  ...
  if(showAngles){
    ...
    if(lm[L.LEFT_SHOULDER].visibility>0.3&&lm[L.LEFT_ELBOW].visibility>0.3&&lm[L.LEFT_WRIST].visibility>0.3){
      const a=ang(lm[L.LEFT_SHOULDER],lm[L.LEFT_ELBOW],lm[L.LEFT_WRIST]),ex=transformX(lm[L.LEFT_ELBOW].x),ey=transformY(lm[L.LEFT_ELBOW].y);
    ...
    if(lm[L.LEFT_SHOULDER].visibility>0.3&&lm[L.LEFT_HIP].visibility>0.3&&lm[L.LEFT_KNEE].visibility>0.3){
      const a=ang(lm[L.LEFT_SHOULDER],lm[L.LEFT_HIP],lm[L.LEFT_KNEE]),ex=transformX(lm[L.LEFT_HIP].x),ey=transformY(lm[L.LEFT_HIP].y);
    ...
    if(lm[L.RIGHT_HIP].visibility>0.3&&lm[L.RIGHT_KNEE].visibility>0.3&&lm[L.RIGHT_ANKLE].visibility>0.3){
      const a=ang(lm[L.RIGHT_HIP],lm[L.RIGHT_KNEE],lm[L.RIGHT_ANKLE]),ex=transformX(lm[L.RIGHT_KNEE].x),ey=transformY(lm[L.RIGHT_KNEE].y);
